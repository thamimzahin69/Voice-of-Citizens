  ...
  return (
    <section className="page">
      <header className="page-header">
        <p className="page-eyebrow">Forecasting</p>
        <h1>Predictions</h1>
        <p>View vote outcome projections based on current vote trends.</p>
      </header>

      {loading && <p className="empty-state">Loading predictions...</p>}
        ...
        {predictions.length === 0 && !loading ? (
          ...
          predictions.map((item) => (
            <Card key={item.electionId} title={item.electionTitle}>
              <p><strong>Leading candidate:</strong> {item.leadingCandidate}</p>
              <p><strong>Chance to win:</strong> {item.winProbability}%</p>
            </Card>
          ))
        )}
      </div>
